...
server <- function(input, output) {
  ...
  observe({
    ...
    # Setting up for choropleth
    palette <- colorNumeric("YlOrRd", densities)
    
    # Setting up for hover tooltips
    labels <-  sprintf("<strong>%s</strong><br/ >%g unique APs",
                       locationBinnedPop$location[which(coord$location %in% input$include)], # location
                       locationBinnedPop$pop[which(coord$location %in% input$include)]) %>% # number of unique APs
