...
# Global Vars
apsdf <- read_csv("./Buildings/Perkins/apData.csv")
wallsdf<- read_csv("./Buildings/Perkins/buildingData.csv")
...
refresh = 1 # Refresh rate in minutes
  ...
  ready <- left_join(fortified, chartData, by = c("id" = "ap"))
    ...
    buildingMap <- ggplot() + geom_polygon(data = ready, aes(fill = Utilization, x = long, y = lat, group = group)) +
